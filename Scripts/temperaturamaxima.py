...
def ler_tmin(arquivo):
    ...
    tmin = ds.variables['tmax'][:]
    tmin = np.where(tmin == ds.variables['tmax']._FillValue, np.nan, tmin)
    ds.close()
    return tmin
pasta_dados_diarios = 'jul max/'
climatologia_janeiro = ler_tmin('SAMeT_CPTEC_TMAX_mean_jul.nc')
...
with nc.Dataset('SAMeT_CPTEC_TMAX_mean_jul.nc') as ds:
...
plt.title('Anomalia de Temperatura Máxima - Julho 2024')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.savefig('anomalia_temperatura_maxima_jul.png', dpi=300, bbox_inches='tight')
plt.show()
